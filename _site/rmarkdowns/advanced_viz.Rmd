...
```{r, warning = FALSE}
goal_4_histogram <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d(option = "magma") +
  labs(title = "Distributions of SDG 4 Scores",
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("PRGn", goal_11_score)(goal_11_score), label = mytext)
```
